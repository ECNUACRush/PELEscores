...
class Chest(data.Dataset):
    ...
    def __init__(self, prefix, phase, transform_params=dict(), sigma=5, num_landmark=6, size=[512, 512], use_abnormal=True, chest_set=None, exclude_list=None,use_background_channel=False):
        ...
        n = len(files)
        # random.shuffle(files) cant shuffle in this area, other will be overfitting

        train_num = round(n*0.8)   # round(n*0.7) # 180  
        # 0.4 
        val_num = round(n*0.05)  # round(n*0.1)  # 24
        test_num = n - train_num - val_num
        if phase == 'train':
            self.indexes = files[-train_num:]
        elif phase == 'validate':
            self.indexes = files[:val_num]
        elif phase == 'test':
            self.indexes = files[val_num:val_num+test_num]
        else:
